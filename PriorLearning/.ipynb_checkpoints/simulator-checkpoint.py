...
class InitializePriorFamily:
    def __init__(self, rep, parameters_dict):
    ...
    def __call__(self):
        ...
        n = -1
        for j, param_name in enumerate(self.parameters_dict.keys()):
            # prior distribution family of model parameter
            prior_family = self.parameters_dict[param_name]["family"]
            ...
            for i, hyperparam_name in enumerate(hypparam_name):
                initialization = self.parameters_dict[param_name]["initialization"][i]
        ...
        **kwargs : optional keyword arguments
        ...
        if (self.method == "ideal_expert_nf" or self.method == "ideal_expert"):
            ...
            if self.method == "ideal_expert_nf":
                for param_name in self.parameters_dict.keys():
                    print(param_name)
                    if param_name == "sigma" or param_name.startswith("omega"):
                        print("transform applied")
                        priors.append(tf.math.log(
                            self.parameters_dict[param_name]["true"].sample((self.B,self.rep))))  
                    else:
                        priors.append(
                            self.parameters_dict[param_name]["true"].sample((self.B,self.rep)))  
        ...
        if self.method == "hyperparameter_learning":
            ...
            for initial_hyperparameter, name in zip(self.initialized_priors,
                ...
                prior_family = self.parameters_dict[name]["family"]
        ...
        if self.method == "normalizing_flow":
            ...
            base_dist = tfd.Normal(
                loc = tf.zeros(len(self.parameters_dict)),
                scale = tf.ones(len(self.parameters_dict))
                ).sample((self.B, self.rep))
